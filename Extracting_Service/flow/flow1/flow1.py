...
def main():
        ...
        try:
            ...
            if not step4_result["success"]:
                ...
                if DELETE_EML_AFTER_PROCESS:
                    logger.info(f"  === Step 5: Delete EML File from S3 ===")
                    ...
                    if step5_result["success"]:
                        ...
                        logger.warning(f"  WARNING: Processed but failed to delete from S3: {s3_key}")
                        results.deletion_failed_count += 1
                else:
                    logger.info(f"  ℹ Skipped deletion (DELETE_EML_AFTER_PROCESS=false): {s3_key}")
    ...
    if DELETE_EML_AFTER_PROCESS and results.deletion_failed_count > 0:
        logger.info(f"Deletion failures: {results.deletion_failed_count}")
